...
3. **배포 규칙** — 배포는 반드시 아래 두 경우에만 실행한다. 그 외 어떤 상황에도 배포를 실행하거나 제안 없이 트리거하지 않는다.
   ...
   **🔒 비공개 단계 정책 (2026-06-01~)**: 마켓레이더 v5는 **서비스가 완벽히 돌기 전까지 사용자에게 오픈하지 않는다.** 사용자 0명 시점이라 즉시 배포 필요성 없음. P0/P1 머지 후 **자동 즉시 배포 금지** — main 누적 후 아래 이벤트 트리거 충족 시 일괄 1회 배포.

   | 이벤트 트리거 | 추정 빈도 |
   |--------------|----------|
   | ① 첫 외부 사용자 노출 직전 (런칭) | 미래 1회 |
   | ② 사용자 인터뷰/시연 직전 | 사이클당 1회 |
   | ③ 큰 변화 단위 완결 (데이터 신뢰 트랙, 디자인 방향 prototype 등) | 1~2주에 1회 |
   | ④ 대표가 production에서 직접 확인하고 싶을 때 | on-demand |

   → 머지는 자율, 배포는 트리거 충족 + 대표 확인. 트리거 없이 P0/P1 머지만으로 "배포할까요?" 자동 제안 금지.
