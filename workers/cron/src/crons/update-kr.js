// crons/update-kr.js — 국장 가격 갱신 (CF Workers 이식)
...
async function fetchHantooFallback(env) {
  // 한투 토큰이 없으면 fallback 불가
  const appKey = env.HANTOO_APP_KEY;
  const appSecret = env.HANTOO_APP_SECRET;
  if (!appKey || !appSecret) return null;

  // 한투 토큰 발급
  const token = await getHantooToken(env);
...
export async function updateKr(env) {
  ...
  try {
    ...
    try {
      ...
      try {
        const hantooItems = await fetchHantooFallback(env);
        if (hantooItems) {
          items = hantooItems;
          source = 'hantoo';
        } else {
