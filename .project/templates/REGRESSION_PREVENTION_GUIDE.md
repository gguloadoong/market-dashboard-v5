...
### 4. PR 전 독립 코드 리뷰 — 자기 리뷰는 무효

코드를 작성한 맥락에서 스스로 리뷰하면 관대해진다.
PR을 만들기 전에 코드 작성 맥락을 모르는 별도 에이전트에게 리뷰를 맡긴다.

**Claude Code에서 적용하는 방법:**

```
# 1. PR 브랜치 작업 완료 후
git diff main...HEAD > /tmp/pr-diff.txt

# 2. 별도 code-reviewer 에이전트 호출 (Claude Code Agent 기능)
#    → 이 에이전트는 코드가 어떻게 작성됐는지 맥락을 모름
#    → 오직 diff만 보고 판단

# 3. 크리티컬/버그 지적 → 수정 후 재리뷰
...
**CLAUDE.md에 추가할 규칙:**
```
PR 생성 전 필수: code-reviewer 에이전트에 git diff 전달하여 독립 리뷰.
코드 작성자가 직접 리뷰하는 것은 무효.
```

**왜 별도 에이전트인가?**
- 같은 대화 맥락에서 리뷰하면 "내가 이렇게 만든 이유"를 알고 있어서 방어적으로 판단한다
- 별도 에이전트는 diff만 보므로 구현 의도에 영향받지 않는다
- 봇 리뷰(Gemini, CodeRabbit)는 할당량 초과 시 대체 불가 → 독립 에이전트는 항상 가능
...
[ ] 7. CLAUDE.md에 "PR 전 code-reviewer 에이전트 독립 리뷰 필수" 추가
```

4개 이상 완료하면 반복 회귀 발생률이 크게 줄어든다.
...
> **자기 리뷰는 무효다. PR 전에 맥락을 모르는 에이전트가 diff를 검토해야 한다.**
