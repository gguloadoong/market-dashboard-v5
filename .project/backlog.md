...
## ⏸️ 보류 — Phase 8 홈 리디자인 (2026-04-20)

**결론: 진행하지 않음.**

**배경:**
- 이지원(자율주행 기간 2026-04-07) 리서치가 "섹션 10개로 산만" 진단 후 3안 제안
- 그러나 실제 v5 홈은 이미 Phase 8A/8B + "시안C 커맨드센터 10→5섹션" + "15→9위젯" 등 **여러 차례 개편의 결과물**
- 리서치 진단 근거(10개 산만)가 실제와 불일치 (현재 5~6섹션)

**기각 사유:**
- 실 사용자 피드백·정량 지표 없음 (로그인 없어 측정 불가)
- "이론적 완벽주의 제안"에 가까움 — 재개편 피로 리스크
- 대표 확인: "이 작업을 한번 한 게 지금의 v5 아니야?" → 맞음

**남긴 자료 (향후 근거 생기면 참고):**
- `.project/home-redesign-research.md` — 이지원 경쟁사 분석 + 원칙 5개
- `.project/wireframes/` — PC(pc-*.html) + Mobile(option-*.html) 각 3안 HTML 목업
- 이 중 A안의 일부(관심종목 첫방문자 숨김 등) 후속 퀵윈으로 적용 검토 가능

**재검토 조건:**
- 실 사용자 피드백 수집 장치 가동 후 3개월 이상 데이터 누적, 또는
- 특정 UX 지점에서 이탈률·혼란 징후가 구체적으로 관측될 때
...
| 2026-04-28 | [P2-20] 적중률 높은 시그널 섹션 — SignalBoardWidget 인라인 (PR #242) |
| 2026-04-28 | [P2-21] 관심종목 이상 신호 스트립 — WatchlistMini 인라인 (PR #237) |
| 2026-04-28 | [P2-22] 내러티브 시그널 — narrativeBuilder.js + 시그널/뉴스/섹터 자동 연결 (PR #243) |
| 2026-04-28 | [P2-23] 데이터 소스 헬스 모니터 cron — fallback 체인 감시 + Issue 자동 생성 (PR #239) |
...
## 🟠 Phase 9: 시그널 신뢰성 + 매수 결정 직전 5분 완결성 (2026-04-28 스프린트)

> **목표**: "시그널을 판다"(ADR-016)의 마지막 1마일 — 사용자 신뢰 가시화 + 5분 결정 흐름 완결
> **기준**: 매수 결정 직전 5분을 돕는가 + DAU를 높이는가 + 경쟁사가 못하는가 + 현재 스택으로 구현 가능한가
> **리서치**: 2026-04-28 (이지원 Strategy + Phase 1~8 전소진 후 신규 P2 발굴)
> **경쟁사 갭**: 토스증권 — 자체 시그널/적중률 없음. 업비트 — 코인 한정. 키움 — 5분 결정 흐름/신뢰 표면 없음.
> **완료**: P2-20·21·22·23 전항목 완료 (2026-04-28)

### ~~[P2-20] 시그널 정확도 트래커 + 신뢰 배지~~ ✅ 완료 (2026-04-28 PR #242)
- 적중률 높은 시그널 섹션 — SignalBoardWidget 인라인 구현

### ~~[P2-21] 관심종목 모닝 브리핑 (3시장 통합 1분 요약)~~ ✅ 완료 (2026-04-28 PR #237)
- 관심종목 이상 신호 스트립 — WatchlistMini 인라인 구현

### ~~[P2-22] 내러티브 시그널 — 시그널/뉴스/플로우 자동 연결~~ ✅ 완료 (2026-04-28 PR #243)
- narrativeBuilder.js 신규 + 시그널/뉴스/섹터 자동 연결 컨텍스트 구현

### ~~[P2-23] 데이터 소스 리스크 헬스 모니터~~ ✅ 완료 (2026-04-28 PR #239)
- health-check.js cron — fallback 체인 감시 + 임계값 위반 시 GitHub Issue 자동 생성

---

## 🟠 Phase 10: 시그널 신뢰 → 행동 전환 + 알고리즘 부채 청산 (2026-04-29 스프린트)

> **목표**: Phase 9에서 가시화한 시그널 신뢰를 사용자 행동(클릭→결정)으로 변환하고, 알고리즘 부채 3건을 정리해 신뢰 토대를 굳힌다
> **기준**: 5분 결정 흐름 단축 + DAU 체류시간 증가 + 알고리즘 데이터 정합성
> **리서치**: 2026-04-29 (이지원 Strategy + Analyst Opus)
...
### ~~[P3-3] 시그널 신뢰 배지 표본·임계값 룰 확립~~ ✅ 완료 (2026-04-29 PR #251)
- N≥30 통일, "데이터 누적 중" 표기, sort comparator antisymmetry 버그 수정

### [P3-4] 시그널 → 1탭 결정 패널 (인라인 확장)
- 시그널 카드 탭 시 인라인 확장: 왜·뉴스 1줄·미니차트·관심종목 추가 버튼
- **완료 기준**: 시그널→결정 평균 탭수 ≤ 3, 모바일 375px 깨짐 없음
