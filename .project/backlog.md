...
### [P2-20] 시그널 정확도 트래커 + 신뢰 배지
- **배경**: 시그널 코어(965줄, 88 테스트)는 완성됐지만 "이 시그널 믿을만한가?" 사용자 신뢰 표면 부재. ADR-016 "시그널을 판다"의 마지막 1마일. 경쟁사 어디도 자체 시그널 적중률 미공개.
- **기능**:
  1. 시그널 발화 시 `{type, symbol, firedAt, priceAtFire}` Redis 기록
  2. 30분/4시간/24시간 후 cron이 후속 가격 측정 → 타입별 윈도우 적중률 계산
  3. 시그널 카드에 "이 타입 최근 30일 적중률 N%" 배지 (TYPE_META 통합)
  4. 시그널 상세 패널에 최근 결과 미니 히트맵
  5. 홈에 "오늘의 적중 시그널" 위젯
- **난이도**: 중간 (`signal-accuracy.js` 기반 + Redis 기록 cron 추가)
- **완료 기준**: 시그널 카드 적중률 배지 표시 (샘플 < 10건 → "추적 중"), 24시간 후 cron 정상 동작, 단위 테스트 20건+

### [P2-21] 관심종목 모닝 브리핑 (3시장 통합 1분 요약)
- **배경**: JTBD 핵심("관심종목 중 뭔가 일어나고 있나?") 충족이 현재 알림 의존적. 앱 진입 후 3초 안에 답해주는 화면 부재. `morning-briefing.js` API 활용도 낮음. 3시장 통합 관심종목 브리핑은 우리만 가능.
- **기능**:
  1. 홈 최상단 "오늘 내 관심종목 요약" 카드 1개
  2. 4 영역: ① 큰 변동 ±3% 이상 ② 시그널 발화 ③ 임팩트 뉴스 ④ 다가오는 이벤트 (각 0~3건)
  3. 1줄 헤드라인 자동 생성 (규칙 기반, AI 없이)
  4. 관심종목 0개 시 "전체 시장 1분 브리핑"으로 fallback
- **난이도**: 낮음 (기존 데이터 조합 + UI 신규)
- **완료 기준**: 관심종목 1개 이상 시 홈 최상단 브리핑 카드, 4 영역 각 0~3건, 모바일 375px 1스크롤 이내

### [P2-22] 내러티브 시그널 — 시그널/뉴스/플로우 자동 연결
- **배경**: 시그널 발화 시 "왜?"를 사용자가 직접 조립 → 5분 결정 흐름 단절. 다변수 통합 내러티브는 우리만 가능.
- **기능**:
  1. 시그널 발화 시 ±15분 뉴스·동조 섹터·KR flow·거래량 자동 매칭
  2. 규칙 기반 내러티브 합성 (AI 없이 결정적 작동): "삼성전자 외인 +1,200억 + 'HBM 수주' 뉴스 + SK하이닉스 동반 +2.1% → 반도체 섹터 동조"
  3. 시그널 카드 확장에 "🧩 컨텍스트" 영역 (3줄 이내, 매칭 부족 시 숨김)
- **난이도**: 중간~높음 (규칙 엔진 + 한국어 템플릿 합성기 신규 작성)
- **완료 기준**: 4종 데이터 매칭 정상 동작, 내러티브 60자 이내, `narrative-renderer.test.js` 20건+

### [P2-23] 데이터 소스 리스크 헬스 모니터
- **배경**: Yahoo v8 비공식 엔드포인트 차단 예측 불가, RSS 간헐적 실패 누적, 자동 알림 부재. 실제 사고 이력: 2026-03-29 v7→v8 강제 교체.
- **기능**:
  1. 일별 cron이 fallback 체인 각 단계 응답 시간/성공률 측정
  2. RSS 소스별 실패율 24시간 집계
  3. 임계값 위반(실패율 ≥ 30%) 시 GitHub Issue 자동 생성
- **난이도**: 낮음~중간 (cron 1개 + GitHub API)
- **완료 기준**: `/api/cron/health-check` 일 1회, 임계값 위반 시 자동 Issue 생성 (ai-generated + ops/p1)
