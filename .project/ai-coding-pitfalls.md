...
### 27. 외부 경계 문자열 키 계약 — diff 밖 생산자에 의존하나 검증 불가·조용히 미발화 (Cross-Boundary String-Key Contract — Unverifiable Out-of-Diff Producer, Silently Inert)

**우리 사례** (2026-06-09 코드 리뷰, #380 데이터 freshness 'N분 전 업데이트'): `api/_price-cache.js`의 `getCronLastOk()`가 Redis 키 `cron:lastOk:{kr,us,coins}`를 mget으로 읽어 `asOf`를 만든다. 이 키를 *쓰는* 쪽은 이 diff에 없는 워커 크론(`recordCronSuccess`)이다. 소비자(읽기) 코드는 완결·정확하고 `RelativeTime`까지 배선됐다. 그러나 그 정확성은 *diff 밖에서 독립 배포되는* 생산자가 **정확히 같은 키명·형식**으로 써준다는 가정에 전적으로 의존한다. 크론이 다른 키명(예: `cron:ok:kr`)이나 다른 값 형식으로 기록하면 mget이 전부 미스 → `asOf=null` → `RelativeTime`이 null 렌더 → "N분 전" 배지가 *조용히 사라진다*(크래시 없음, graceful). 로컬 빌드·테스트·타입체크는 전부 통과하고, 리뷰 diff만으로는 계약 충족 여부를 *검증할 수 없다*. VERDICT는 PASS([HIGH] 비차단, 머지 전 실제 Redis에서 `cron:lastOk:kr` 존재 확인 권장)였으나, 검증 불가능한 외부 경계 계약 1건.

**근본 원인**: AI는 분산 경계(다른 프로세스·다른 배포 단위·다른 레포)를 잇는 계약을 *맨 문자열 리터럴* 하나로 결합한다(`redis.get('cron:lastOk:kr')`, 큐 이름, 이벤트 토픽, S3 경로, 환경변수 키). 소비자 코드를 작성할 때 "반대편이 이 키를 정확히 이 이름·형식으로 쓴다"를 *이미 성립한 사실*로 가정하고 happy-path를 완성한다. 그러나 (1) 그 생산자가 *이 diff/레포 안에 없어 grep으로도 안 잡힌다*는 점, (2) 두 반쪽을 잇는 *컴파일타임/타입/공유상수 링크가 0*이라 미스매치가 정적 분석·테스트·리뷰 어디에도 안 걸린다는 점, (3) 미스매치 시 graceful null로 *조용히 미발화*해 "동작 안 함"이 "데이터 없음"으로 위장된다는 점을 모델링하지 않는다.

**발생 패턴**:
```js
// AI가 자주 생성 (잘못됨 — diff 밖 생산자에 맨 문자열로 의존, 검증 불가)
// 소비자 (이 PR)
const [kr, us, coins] = await redis.mget(
  'cron:lastOk:kr', 'cron:lastOk:us', 'cron:lastOk:coins'  // ← 이 키를 쓰는 곳은 이 diff에 없음
);
const asOf = { kr: num(kr), us: num(us), coins: num(coins) }; // 미스매치 시 전부 null → 조용히 미발화
// 생산자(다른 워커·다른 배포)가 정말 'cron:lastOk:kr'에 epoch ms로 쓰는가? — diff로 확인 불가

// 올바른 패턴 — 계약을 한 곳에 못박고, 미스매치를 관측 가능하게
// 1) 키·형식을 공유 상수/헬퍼로 추출 → 생산자·소비자가 같은 심볼 import (drift 컴파일타임 차단)
export const cronLastOkKey = (m) => `cron:lastOk:${m}`;
// 2) 생산자가 diff 밖이면 머지 전 실제 Redis에 그 키가 쓰이는지 1회 검증(계약 테스트)
// 3) 전부 null이면 단순 빈 데이터가 아니라 "계약 미충족" 신호로 구분 (조용한 미발화 차단)
```

**왜 #16·#2·#5와 다른가**:
- #16(생산자 부분 이관)은 생산자들이 *이 코드베이스 안에 있고*(grep 가능) 기존 필드를 *마이그레이션*하다 일부만 신포맷으로 바꾼 비대칭이다. 여기선 생산자가 *이 diff/레포 밖*(독립 배포 워커 크론)이라 grep에도 안 잡히고, 마이그레이션이 아니라 *신규 계약*을 한쪽만 작성한 것이다(내부 부분이관 vs 외부 경계 신규계약).
- #2(Orphaned)는 *생산* 쪽이 호출자 없이 떠 있는 것. 여기선 소비자가 완전 배선됐고 *생산자가 외부에 있어 검증 불가*하다(미연결 생산자 vs 검증불가 외부 생산자).
- #5(조용한 실패)는 `catch`가 에러를 *삼켜* 관측 불가다. 여기선 에러가 없고 *계약 미스매치가 graceful null로 정상처럼 처리*돼 "미발화"가 "데이터 없음"과 구분 안 되는 것이다(에러 은폐 vs 계약 미충족 위장).

**영향**:
- diff만으로 계약 충족 검증 불가 → 리뷰 PASS·빌드·테스트 통과해도 프로덕션에서 배지가 *영구 미발화* 가능
- 맨 문자열 결합이라 키명·형식 drift가 정적 분석·타입·테스트 어디에도 안 걸림
- graceful null이 "기능 죽음"을 "데이터 없음"으로 위장 → 한참 뒤 QA·사용자가 "왜 N분 전이 안 뜨지"로 발견
- 생산자가 다른 배포 주기를 타면, 한쪽만 키명을 바꿔도 무성(無聲) 회귀

**해결책**:
- ✅ 분산 경계 키(Redis 키·큐명·이벤트 토픽·env 키·경로)는 *공유 상수/헬퍼*로 추출해 생산자·소비자가 같은 심볼을 import (맨 문자열 중복 금지 → drift 컴파일타임 차단)
- ✅ 생산자가 diff 밖이면 "계약 검증"을 리뷰 항목으로 명시 — 머지 전 실제 저장소(Redis 등)에서 키 존재·형식 1회 확인
- ✅ "전부 null/미스" 상태를 *정상 빈 데이터*와 구분 가능하게 — 최소 1회 `console.warn`이나 헬스 신호로 관측 가능화(조용한 미발화 차단)
- ✅ 리뷰 체크리스트: "이 코드가 의존하는 키/이벤트/토픽을 *쓰는* 쪽이 이 diff 안에 있는가? 없다면 계약(이름·형식)을 무엇이 보장하나? 미스매치는 크래시인가 조용한 미발화인가?"

---

### 28. 기계 파싱 지시문을 산문으로 작성 — AI 문체 구두점이 지시문을 조용히 무효화 (Prose-Styled Machine Directive — AI Typography Silently Disarms the Directive)

**우리 사례** (2026-06-11 코드 리뷰, #400 정보구조 명확화): 과거 AI가 작성한 `// eslint-disable-line <룰명> — 설명` 주석이 코드베이스에 있었다. ESLint의 지시문-설명 구분자는 ` -- `(공백+쌍하이픈+공백)인데, AI는 산문 습관대로 em-dash(`—`)를 썼다. ESLint는 em-dash를 구분자로 인식하지 못해 "룰명 — 설명" 전체를 룰 이름 목록으로 파싱 → 어떤 룰과도 매치 안 됨 → **disable이 처음부터 동작한 적이 없었다.** #400 PR이 ` — `→` -- ` 교체로 발견·수정했고, 리뷰어가 "단순 스타일이 아니라 실효성 수정"이라고 명시한 건이다. 그동안 빌드·테스트·리뷰 전부 통과한 채 잠복해 있었다.

**근본 원인**: AI는 주석을 *산문*으로 생성하며, em-dash는 AI 문체의 시그니처 구두점이다. 그러나 일부 주석은 산문이 아니라 *기계가 파싱하는 코드*다 — `eslint-disable` 계열의 ` -- ` 설명 구분자, `# noqa: E501`의 콜론·코드 형식, `# type: ignore[code]`, coverage 프라그마, shebang, YAML frontmatter. AI는 "지시문 + 사람을 위한 설명"을 자연어 타이포그래피(em-dash, 스마트 따옴표, 전각 문자)로 이어 붙이면서, 그 micro-syntax가 깨지는지 검증하지 않는다. 깨진 지시문은 에러를 내지 않고 *조용히 무효*가 되므로 어떤 게이트에도 걸리지 않는다.

**발생 패턴**:
```js
// AI가 자주 생성 (잘못됨 — em-dash는 ESLint 설명 구분자가 아님)
useEffect(fn, []); // eslint-disable-line react-hooks/exhaustive-deps — 의도적 1회 실행
// → "react-hooks/exhaustive-deps — 의도적 1회 실행" 전체가 룰명으로 파싱 → 아무것도 억제 안 됨

// 올바른 패턴 — 기계 구분자(쌍하이픈 ` -- `)만 인정됨
useEffect(fn, []); // eslint-disable-line react-hooks/exhaustive-deps -- 의도적 1회 실행
```

**영향**:
- 지시문이 "처리됨"을 *주장*하지만 무효 — 억제하려던 경고가 그대로 노출되거나, 설정·다른 장치가 우연히 가려주면 영구 잠복
- em-dash(`—`)와 쌍하이픈(`--`)은 사람 눈에 거의 구분 안 됨 → 코드 리뷰로도 못 잡고, 빌드·테스트는 애초에 검사 대상이 아님
- 깨진 "지시문+설명" 관용구가 복사·확산되면 코드베이스의 억제 지시문 전체가 장식이 됨

**왜 #20·#26·#22와 다른가**:
- #20(중복 억제 지시문)은 지시문이 *문법적으로 유효*하나 설정이 이미 처리해 *불필요*한 것(잉여가 문제). 여기선 지시문이 *필요*한데 구두점이 문법을 깨뜨려 *무효*다(잉여 vs 고장).
- #26(환상 근거 주석)은 기계가 읽지 않는 주석의 *내용(서사)*이 허구다. 여기선 주석 자체가 *기계가 읽는 코드*이고 그 *형식*이 깨졌다(내용 거짓 vs 형식 고장).
- #22(위치 셀렉터 맹점)와 "의도를 주장하지만 런타임 무효"라는 결과는 같으나, #22는 셀렉터의 *매칭 컨텍스트*(DOM 형제 구조) 오판이고, 여기선 *주석 micro-syntax*를 산문 타이포그래피로 오염시킨 것 — 깨진 레이어가 다르다.

**해결책**:
- ✅ 지시문 주석(`eslint-disable`, `noqa`, `ts-expect-error`, 프라그마)에 설명을 붙일 땐 해당 도구의 *공식 구분자 문법*을 확인 — ESLint는 ` -- `만 인정
- ✅ 기계가 파싱하는 주석 안에서는 ASCII 구두점만 사용 — em-dash·스마트 따옴표·전각 문자 금지
- ✅ ESLint `reportUnusedDisableDirectives` 유지 — 룰명이 깨져 아무것도 억제하지 못하는 disable을 unused로 검출할 보조선
- ✅ 리뷰 체크리스트: "기계가 파싱하는 주석에 산문 구두점(`—`, `"`, `'`)이 섞여 있는가? 이 지시문이 실제로 동작하는지(룰명 파싱·억제 여부) 확인했는가?"

---

*이 문서는 마켓레이더 v5 개발 과정에서 직접 경험한 사례를 바탕으로 작성되었다. (마지막 업데이트: 2026-06-11)*
