#!/usr/bin/env bash
# pre-deploy-consensus.sh — 배포 전 QA·개발팀·조직장 컨센서스 게이트
#
...
#   3. QA 승인 (장성민 — 품질 기준 검증)
#   4. 개발팀 승인 (박서연 FE / 김민준 BE)
#   5. 조직장 승인 (이준혁 CPO — 사업 방향 확인)
...
# ── Gate 2: P0/P1 이슈 없음 ──────────────────────────────────────────────────
echo -e "${BLUE}[2/5] P0/P1 오픈 이슈 확인${NC}"
...
# ── Gate 3: QA 승인 (장성민) ─────────────────────────────────────────────────
echo -e "${BLUE}[3/5] QA 승인 — 장성민 (품질 기준 검증)${NC}"
...
# ── Gate 4: 개발팀 승인 ──────────────────────────────────────────────────────
echo -e "${BLUE}[4/5] 개발팀 승인 — FE(박서연) / BE(김민준)${NC}"
...
# ── Gate 5: 조직장 승인 (이준혁 CPO) ─────────────────────────────────────────
echo -e "${BLUE}[5/5] 조직장 승인 — 이준혁 CPO${NC}"
# CLAUDE.md의 배포 방향과 일치하는지: 배포 조건 (P0/P1 수정 or 주요 기능 완료) 확인
DEPLOY_CONDITION_MET=0

# P0/P1 수정이 있었나?
RECENT_FIX=$(git log origin/main..HEAD --oneline 2>/dev/null | grep -c "fix:" || echo "0")
# 주요 기능 완료 (feat:) 커밋?
RECENT_FEAT=$(git log origin/main..HEAD --oneline 2>/dev/null | grep -c "feat:" || echo "0")
