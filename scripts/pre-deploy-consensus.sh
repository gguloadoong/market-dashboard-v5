...
## 배포 예정 커밋 (이 목록만 실제 배포됩니다)
${DEPLOY_COMMITS}

## 미머지 오픈 PR (참고용 — 배포되지 않음, 존재만 인지)
⚠️ 아래 PR들은 feature 브랜치에만 있으며 main에 머지되지 않았습니다. 배포 예정 커밋에 포함되지 않습니다.
${OPEN_PRS:-없음}
...
검토 기준:
- '배포 예정 커밋' 목록에 있는 커밋만 판단 대상입니다. 오픈 PR 커밋은 배포되지 않습니다.
